...
-   $X_6$ : Región de residencia del asegurado en EE.UU.: noreste,
    sureste, suroeste o noroeste.
...
femenino, no fumador y región noreste). Sin embargo, este valor no
...
En cuanto a la variable `region`, los coeficientes representan
diferencias respecto a la región de referencia (noreste):

-   Vivir en el noroeste (`regionnorthwest`) reduce en promedio los
    costos en 353.0 unidades.\
-   Vivir en el sureste (`regionsoutheast`) reduce los costos en 1035.0
    unidades.\
-   Vivir en el suroeste (`regionsouthwest`) reduce los costos en 960.0
    unidades.
...
grupos, por lo que podría considerarse una transformación (como el
...
significativa ($p = 8.446 \times 10^{-5}$). Por otro lado, la variable
`region` presenta un valor-p de $0.096$, lo que sugiere que su efecto
global es más débil en comparación con las demás. En conjunto, este
análisis confirma que el modelo ajustado tiene capacidad explicativa
relevante para predecir los costos médicos.
...
Finalmente, respecto a la región geográfica, `regionnorthwest` no es
significativa ($p = 0.459$), mientras que `regionsoutheast`
($p = 0.0308$) y `regionsouthwest` ($p = 0.0448$) presentan efectos
negativos moderados, sugiriendo diferencias regionales leves en los
costos médicos con respecto a la región de referencia.
...
mientras que el sexo y algunas regiones no muestran un efecto
...
especificadas (edad 40 años, BMI 28, 2 hijos, hombre, no fumador y
residente en la región suroeste). El modelo predice un gasto medio
aproximado de **7692.77 unidades monetarias**. Además, con un nivel de
confianza del 95%, se concluye que el verdadero costo promedio para
personas con este perfil se encuentra entre **6887.33** y **8498.21**.
...
fumador y residente en la región suroeste). El modelo predice un gasto
aproximado de **7692.77 unidades monetarias**, pero debido a la
