...
|--------|------------|------|
| `charges` | Costo del seguro médico | Cuantitativa continua (respuesta) |
| `age` | Edad del asegurado (años) | Cuantitativa discreta |
| `bmi` | Índice de masa corporal | Cuantitativa continua |
| `children` | Número de hijos dependientes | Cuantitativa discreta |
| `sex` | Sexo del asegurado | Cualitativa nominal |
| `smoker` | Condición de fumador | Cualitativa dicotómica |
| `region` | Región geográfica | Cualitativa nominal |
...
```{r}
# Dado que los datos corresponden a observaciones individuales y no a series temporales, se asume independencia de los errores.
```
...
```{r}
# Ejemplo de transformación logarítmica (opcional)
...
```{r}
# La prueba F se obtiene del resumen del modelo
summary(modelo)
```
...
```{r}
# Las pruebas t individuales se obtienen del resumen del modelo
summary(modelo)
```
